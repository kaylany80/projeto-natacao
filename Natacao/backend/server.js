...
app.post("/alunos", (req,res) => {
    ...
    if (!nome || !idade || !nivel || !horario ) {
        ...
        const verificaSQL = "SELECT * FROM alunos2 WHERE nome = ?";
        ...
        (erro, resultado) => {
            ...
            const inserirSQL = 'insert into alunos2 (nome, idade, nivel, horario) values(?, ?, ?, ?)'
    ...
    app.get("/alunos", (req,res) => {
        db.query(
            "SELECT * FROM alunos2", (erro, resultado) => {
                if(erro) {
                    return res.status(500).json(erro);
                }
                res.json(resultado);
    ...
    app.delete("/alunos/:id", (req, res) =>{
     ...
     db.query("DELETE FROM alunos2 where id = ?", [id], (erro, resultado) => {
        if(erro) {
 return res.status(500).json(erro);
    ...
    app.put("/alunos/:id", (req, res) => {
        ...
        db.query("SELECT ativo FROM alunos2 WHERE id = ?", [id], (erro, resultado) => {
            if (erro) {
                return res.status(500).json(erro);
            }
            if (resultado.lenght === 0) {
            ...
            db.query("UPDATE alunos2 SET ativo = ? WHERE id = ?", [novoStatus, id], (erro) => {
                if (erro){
                    return res.status(500).json(erro);
                }
                res.json({
